...
import {CarGearApi, CarYearApi} from '@/Types/API/CarTypes';
...
const CarLocation = ({navigation}: CarLocationProps) => {
  ...
  const {getData, data} = useApi<CarYearApi['T'], CarYearApi['D']>(
    {
      cnt: 0,
      list: [],
    },
  ...
  return (
    <View>
      ...
      <View style={styles.searchView}>
        ...
        <TextInput placeholder={t('carYearPh')} placeholderTextColor={Theme.color.gray} onChangeText={setText} style={{...styles.textInput, fontSize: fontSize * 16}} />
      </View>
      {data.list.map(v => {
        return (
