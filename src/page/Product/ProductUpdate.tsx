...
export default function ProductUpdate({route: {params}}: ProductUpdateProps) {
    ...
    const {data: ProductInfo, getData: getProductInfo} = useApi<ProductInfoApi['T'], ProductInfoApi['D']>(
        null,
    ...
    const {PostAPI: registerProduct} = usePostSend(productApiSelector(params?.isEdit == null ? editInfo.isEdit : params.isEdit, product.categoryMenu), {
        imageField: 'pt_file',
        mt_idx: user.mt_idx,
        ...productSendData(product),
        pt_idx: params?.pt_idx ?? editInfo.pt_idx,
    });

    const {data: carDetailOptions, getData: getDataCarDetail} = useApi<CarFuelApi['T'], CarFuelApi['D']>(
        {
            cnt: 0,
            list: [],
        },
    ...
    const {data: carHistoryInformation, getData: getDataCarHistory} = useApi<CarFuelApi['T'], CarFuelApi['D']>(
        {
            cnt: 0,
            list: [],
        },
    ...
    const _Button = useCallback(({isOn, setIsOn, text}) => {
        return (
            <TouchableOpacity
                ...
                onPress={() => setIsOn()}>
                <Text color={isOn ? Theme.color.white : Theme.color.gray} fontSize={`${12 * fontSize}`} medium>
                    {text}
                </Text>
            </TouchableOpacity>
        );
    ...
    useEffect(() => {
        ...
        if (params?.isEdit && params.pt_idx) {
            setEditInfo({
                isEdit: params.isEdit,
                pt_idx: params.pt_idx,
            });
            getProductInfo(); // 물품 불러오기
        }

        return () => {};
    }, [isFocused]);
    useEffect(() => {
        if (product.categoryMenu === 'car' || product.categoryMenu === 'motorcycle') {
            ...
            <KeyboardAwareScrollView>
                <View style={styles.viewMainContainer}>
                    ...
                    <View style={styles.contentContainer}>
                        ...
                        <TextInput multiline style={styles.textAreaInput} value={product.content} onChangeText={text => onChangeProduct('content', text)} />
                    </View>
                            ...
                            <View
                                ...
                                {!isNotCar && (
                                    <View style={styles.marginBottom25}>
                                        <Text fontSize={`${12 * fontSize}`} medium style={{marginBottom: getHeightPixel(5)}}>
                                            {t('doors')}
                                        </Text>
                                        <View style={styles.betweenView}>
                                            <_Button isOn={product.pt_door === '2'} setIsOn={() => onChangeProduct('pt_door', '2')} text={t('door2')} />
                                            <_Button isOn={product.pt_door === '4'} setIsOn={() => onChangeProduct('pt_door', '4')} text={t('door4')} />
                                        </View>
                                        <Line isGray />
                                    </View>
                                )}
                                ...
                                {/* 차량 번호 끝자리 */}
                                <TitleInput
                                    title={t(product.categoryMenu === 'car' ? 'carEndNumber' : 'motorEndNumber')}
                                    placeHolder={t('unSelected')}
                                    isSelect
                                    onPress={() =>
                                ...
                                {carDetailOptions.list.map(item => {
                                    ...
                                    const findItem = typeof selectDetail.find(v => v === _Item.idx) === 'string';
                                    return (
                                        <TouchableOpacity
                                            key={_Item.idx}
                                            ...
                                            onPress={() => {
                                                if (findItem) {
                                                    setSelectDetail(prev => prev.filter(v => v !== _Item.idx));
                                                } else {
                                ...
                                {carHistoryInformation.list.map(item => {
                                    ...
                                    const findItem = typeof selectHistory.find(v => v === _Item.idx) === 'string';
                                    return (
                                        <TouchableOpacity
                                            style={styles.checkBoxTouch}
                                            key={_Item.idx}
                                            onPress={() => {
                                                if (findItem) {
                                                    setSelectHistory(prev => prev.filter(v => v !== _Item.idx));
                                                } else {
                                ...
                                <View style={styles.marginBottom25}>
                                    ...
                                    <View style={styles.betweenView}>
                                        <_Button isOn={product.pt_owner === 'Y'} setIsOn={() => onChangeProduct('pt_owner', 'Y')} text={t('yes')} />
                                        <_Button isOn={product.pt_owner === 'N'} setIsOn={() => onChangeProduct('pt_owner', 'N')} text={t('no')} />
                                    </View>
                                    <Line isGray />
                                </View>
                            </View>
...
function productApiSelector(isEdit: boolean, categoryMenuName: categoryMenuTypes['menu'] | null) {
    let result = '';
    ...
    if (isEdit) {
        if (categoryMenuName !== 'car' && categoryMenuName !== 'motorcycle') {
            result += 'modifty.php';
        } else {
            result += 'modify.php';
        }
