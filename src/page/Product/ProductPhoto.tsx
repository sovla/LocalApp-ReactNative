...
export default function ProductPhoto({navigation, route: {params}}: ProductPhotoProps) {
  ...
  const getPhotos = () => {
    CameraRoll.getPhotos({
      ...
      if (params && Array.isArray(params) && !isDone && Array.isArray(value.edges)) {
        let _image: string[] = [];
        let _number: number[] = [];
  ...
  const onPressComplete = () => {
    navigation.navigate('ProductUpdate', {
      imageFile: selectNumber.map(v => ({
        ...
        mime: imageArray[v - 2]?.path ? imageArray[v - 2].mime : imageArray[v - 2].node?.type,
  ...
  const _renderItem = useCallback(
    ({item, index}) => {
      ...
      const onPress = select === -1 ? () => setSelectNumber(prev => [...prev, index + 1]) : () => setSelectNumber(prev => prev.filter(v => v !== index + 1));
