import {View, TouchableOpacity, ScrollView, Image, StyleSheet, ImageSourcePropType} from 'react-native';
...
import {AlertButton, getOpeningTime, onScrollSlide, viewCountCheck} from '@/Util/Util';
...
import {BusinessProfileAPi, BusinessProfileLikeApi} from '@/Types/API/BusinessTypes';

export default function BusinessProfile({navigation, route: {params}}: BusinessProfileProps) {
    const {t} = useTranslation();
    const fontSize = useAppSelector(state => state.fontSize.value);
    const {user} = useAppSelector(state => state);
    const [isShow, setIsShow] = useState<boolean>(false);

    const {data, getData, setData} = useApi<BusinessProfileAPi['T'], BusinessProfileAPi['D']>(
        null,
    ...
    const {PostAPI: sendLike} = usePostSend<BusinessProfileLikeApi, any>('sell_profile_like.php', {
        mt_idx: user.mt_idx as string,
        sell_type: '1',
        sell_idx: params.sell_idx,
    });
                            ...
                            <View style={styles.whiteBoxTopView}>
                                <View style={styles.row}>
                                    <View style={styles.profileContentView}>
                                        <View
                                            ...
                                            <Image source={{uri: data?.busi_profile}} style={styles.profileImage} />
                                        </View>
                                        <View
                                            style={{
                                            ...
                                            openingTime.map((item, index) => {
                                                ...
                                                return (
                                                    <View
                                                        ...
                                                        <Text fontSize={`${14 * fontSize}`}>{t(dayList[index]) + transItem}</Text>
                                                        {index === 0 && (
                                                            <AutoHeightImage
                                                                source={!isShow ? require('@assets/image/arrow_down.png') : require('@assets/image/arrow_up.png')}
                                                                width={getPixel(10)}
                                ...
                                <View style={styles.IconView}>
                                    <View style={{flexDirection: 'row'}}>
                                        <TouchableOpacity>
                                            <AutoHeightImage source={DribbleIcon} width={getPixel(20)} style={{marginRight: getPixel(14)}} />
                                        </TouchableOpacity>
                                        <TouchableOpacity>
                                            <AutoHeightImage source={FacebookIcon} width={getPixel(20)} style={{marginRight: getPixel(14)}} />
                                        </TouchableOpacity>
                                        <TouchableOpacity>
                                            <AutoHeightImage source={InstagramIcon} width={getPixel(20)} style={{marginRight: getPixel(14)}} />
                                        </TouchableOpacity>
                                        <TouchableOpacity>
                                            <AutoHeightImage source={WhatsAppIcon} width={getPixel(20)} />
                                        </TouchableOpacity>
                                    </View>
                                    <TouchableOpacity onPress={onPressLike} style={styles.likeView}>
