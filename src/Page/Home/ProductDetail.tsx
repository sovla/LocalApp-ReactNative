...
export const ProductDetail = ({navigation, route: {params}}: ProductDetailProps) => {
    ...
    const {
        ...
        null,
        isMotor ? 'product_auto_detail.php' : isCar ? 'product_car_detail.php' : 'product_detail.php',
        {
            mt_idx: user.mt_idx,
    ...
    const onPressLike = useCallback(() => {
        likeApi().then((res: any) => {
            if (res?.result === 'false' && res?.msg) {
                ...
                if (res?.data?.like === 'Y' || res?.data?.like === 'N') setisLike(res.data.like);
            }
        });
    }, []);
            ...
            <View
                ...
                <ScrollView onScroll={onScroll}>
                    ...
                    <View style={styles.contentView}>
                        ...
                        {(isCar || isMotor) && (
                            <>
                                <DarkBlueText style={styles.vehiclePriceText} bold fontSize={`${22 * fontSize}`}>
                                    {brPrice(data.pt_price, {isPadding: true})}
                                </DarkBlueText>

                                <TouchableOpacity onPress={onPressShop} style={styles.sellerInfoTouch}>
                                    <View style={styles.rowCenter}>
                                        <AutoHeightImage source={DocumentGrayIcon} width={getPixel(14)} style={styles.marginRight5} />
                                ...
                                {data?.car_option && (
                                    <>
                                        <Text fontSize={`${18 * fontSize}`} medium style={styles.optionTitle}>
                                ...
                                {data?.car_history && (
                                    <>
                                        ...
                                        <ProductDetailOptionBox dataArray={data.car_history} fieldName="history" />
                                    </>
                                )}
                                {/* 차량 포함 옵션 */}
                                {data?.car_detail && data?.car_detail?.filter(v => v.detail_type === 'Y').length > 0 && (
                                    <>
                                        <Text fontSize={`${18 * fontSize}`} medium style={styles.optionTitle}>
                                            {t('includedInformation')}
                                        </Text>
                                        <ProductDetailOptionBox dataArray={data?.car_detail?.filter(v => v.detail_type === 'Y')} fieldName="detail" isOption />
                                    </>
                                )}
                                {/* 오토바이 기본정보 */}
                                {data?.auto_option && (
                                ...
                                {data?.auto_history && (
                                    <>
                                        ...
                                        <ProductDetailOptionBox dataArray={data.auto_history} fieldName="history" />
                                    </>
                                )}
                                {/* 오토바이 포함 옵션 */}
                                {data?.auto_detail && data?.auto_detail?.filter(v => v.detail_type === 'Y').length > 0 && (
                                    <>
                                        <Text fontSize={`${18 * fontSize}`} medium style={styles.optionTitle}>
                                            {t('includedInformation')}
                                        </Text>
                                        <ProductDetailOptionBox dataArray={data?.auto_detail?.filter(v => v.detail_type === 'Y')} fieldName="detail" isOption />
                                    </>
                                )}
                                <Text fontSize={`${14 * fontSize}`} medium style={styles.carInfomationLastText}>
                                    {t('moreInformation')}
                                </Text>
                            </>
                        )}
                    ...
                    <View>
                        ...
                        <Line height={1} />
                        <TouchableOpacity onPress={onPressUserReport} style={styles.tierGuideView}>
                            <MediumText fontSize={`${14 * fontSize}`}>{t('ReportUser')}</MediumText>
                            <Image source={RightBlackIcon} style={styles.rightImage} />
                        </TouchableOpacity>
                        <Line height={1} />
                ...
                <Shadow distance={5}>
                    <View style={styles.footerView}>
                        <View style={styles.footerLeftView}>
                            <TouchableOpacity onPress={onPressLike} style={styles.pressLikeView}>
                                <Image source={isLike === 'Y' ? require('@assets/image/love_white.png') : require('@assets/image/dislike.png')} style={styles.pressLikeImage} />
                            </TouchableOpacity>
                            ...
                            <View>
                                ...
                                <Text color={data.pt_price_check === 'Y' ? Theme.color.aqua_00 : Theme.color.red} fontSize={`${12 * fontSize}`}>
                                    {data.pt_price_check === 'Y' ? t('nego') : t('noPriceOffer')}
                                </Text>
                            </View>
                        </View>
                        <TouchableOpacity
                            disabled={data.chat_btn === 'N'}
                            onPress={onPressChattingTrade}
                            style={{
                                ...styles.footerRightTouch,
                                backgroundColor: data.chat_btn === 'Y' ? Theme.color.blue_3D : Theme.color.gray,
                            }}>
